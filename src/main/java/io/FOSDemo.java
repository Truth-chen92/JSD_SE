 ...
 *
 * java.io.InputStream,OutputStream是所有字节输入流与字节输出流的
 * 超类，里面规定了读写的基础方法（抽象方法）。实际读写不同设备的字节流都
 * 继承自他们。使得我们将来可以用相同的方式利用这些流来完成对不同设备统一
 * 的读写操作。
 *
 * java将流划分为两大类
 * 节点流：真实链接我们程序和数据源的“管道”，负责实际搬运数据。因此链接的
 *      另一端是明确的。读写一定是建立在节点流基础上进行的。
 * 处理流：不能独立存在，必须链接在其他流上，目的是当数据“流经”当前流时对
 *      其做某些加工处理，简化我们对此的相关操作。
 * 节点流一般也称为：低级流     处理流一般也称为：高级流
 * 实际应用中我们往往会通过串联一组高级流并最终链接到低级流上将一个复杂数据
 * 的读写操作以流水线式的加工来完成，这个过程称为流的链接。
 *
...
//        RandomAccessFile fos=new RandomAccessFile("fos.txt","rw");
        /*
            ...
            会先将该文件原有数据全部删除。然后将通过当前流写出的内容全部
            保存到文件中
...
//        fos.write("那仰望的人心底的孤独和叹息。".getBytes("UTF-8"));
        fos.write("我祈祷".getBytes("UTF-8"));
        fos.write("拥有一个最美的心灵和会流泪的眼睛".getBytes("UTF-8"));
